...
public static boolean tablaInstanc, ivabooleanc;
boolean calcula=false;
public  static String listaPorductosc="";
 DecimalFormat fd = new DecimalFormat("0.00");
DefaultTableModel modeloNat;
JTextField txt=new JTextField();
JTextField txtdesc=new JTextField();
JTextField txtcodPro= new JTextField();
public static JTextField txtautocompletec=new JTextField(); 
int fila,colum,Nfilas;
String cantidadchar,descuentoChar;
double precioPro,CantidaPro,subPro,descuenPro;
public static double subtotalc=0,descuentoc=0,ivac=0,totalc=0;
Calendar calendario = Calendar.getInstance();
conexionCompra factc=new conexionCompra();
DateFormat fecha = new SimpleDateFormat("yyyy/MM/dd");
Date fecha1, fecha2;
public  static  TextAutoCompleter textAutoCompleterc ;
public  static  TextAutoCompleter textAutoCompleterProductoc ;
    ...
    public Compra() {
        ...
        textAutoCompleterc = null;
         textAutoCompleterProductoc = null;
         factc.consultarNumeroCompra();
         txtnumfac.setText(String.valueOf(("00"+(factc.getidventa()+1))));
  ...
  ////////////////////////////////////////////////////////////////////////////
      ...
      textAutoCompleterc = new TextAutoCompleter(txtcodigocliente, new 
    ...
    AutoCompleterCallback() {
        ...
        public void callback(Object selectedItem) {
           ...
           char l;
           boolean enc=true;
            factc.autcompleteProveedor(cadena);
            for (int i = 0; i < cadena.length(); i++) {
                l=cadena.charAt(i);
                if(l!='-' && enc)cadena2=cadena2+l;
                else enc=false;
                }
             factc.autllenarProveedor(cadena2);
        } }
      );
///////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////
textAutoCompleterProductoc = new TextAutoCompleter(txtcodPro, new 
    ...
    AutoCompleterCallback() {
        ...
        public void callback(Object selectedItem) {
           ...
           char l;
           boolean enc=true;
            factc.autcompleteProducto(cadena);
            for (int i = 0; i < cadena.length(); i++) {
                l=cadena.charAt(i);
                if(l!='-' && enc)cadena2=cadena2+l;
                else enc=false;
                }
             factc.autllenarProducto(cadena2);
        } }
      );
...
/////////////////////////////////////////////////////////////////////
        ...
        txt.addKeyListener(new KeyAdapter()
         ...
         if(tblProductosfac.getValueAt(tblProductosfac.getSelectedRow(), 3)!=null &&(caracter<'a' || caracter>'z') && (caracter<'A'||caracter>'Z') ){
                ...
                CantidaPro=0; 
                    e.consume();}
                    ///stock ??? :P 
               factc.consultarProductoStock(modeloNat.getValueAt(fila,1).toString());
                  if((CantidaPro+factc.getstock())>factc.getstock_max()){
                        JOptionPane.showMessageDialog(null,"la cantidad es mayor al stock maximo"+"\n Cantidad ingresada: "+CantidaPro+"\n stock: "+factc.getstock()+"\n stock_max: "+factc.getstock_max());
                        e.consume();
    ...
    private void initComponents() {
        ...
        tblProductosfac.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Línea", "Código", "Descripcion", "Bodega", "Cantidad", "Unidad", "costo Unit", "Descuent", "Sub total"
            }
        ) {
        ...
        btneliminarLinea.setText("ElimLin");
    ...
    private void btnListarClienteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnListarClienteActionPerformed
        lstProveedores dialog = new lstProveedores(new javax.swing.JFrame(), true);
   ...
   dialog.setVisible(true);    
    ...
    private void btnagreProductoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnagreProductoActionPerformed
       lstProductosCompra dialog2 = new lstProductosCompra (new javax.swing.JFrame(), true);
...
if(chekiva.isSelected()) ivabooleanc=true;
else ivabooleanc=false;
calculador();
...
if(totalc<=0)JOptionPane.showMessageDialog(null, "no hay cantidad que facturar :( ");
   else {   
if(mirartabla())JOptionPane.showMessageDialog(null, "problemas con la tabla :P ");
    //fechaemision fechavence subtotal descuento iva totalcobrado totalcobrar idcliente
       ...
       calculador();
        ...
        factc.ingresarCompra(fecha.format(fecha1), fecha.format(fecha2), subtotalc, descuentoc, ivac, totalc, cadena2,txtobserva.getText());
          modificarProductos();
...
lstTerminoPagoCompra dialog3 = new lstTerminoPagoCompra(new javax.swing.JFrame(), true);
   dialog3.setLocationRelativeTo(null);
   dialog3.setVisible(true); 
    if (!txttermino.getText().equals("Contado")){
        factc.consultarTermino(txttermino.getText());
           Calendar calenfin = Calendar.getInstance();
           calenfin.setTime(fechaEmision.getDate());
     calenfin.add(Calendar.MONTH, factc.getNmeses()*factc.getnperiodo());
       ...
       // JOptionPane.showMessageDialog(null, "hay vamos :  ;) ");
        if (tblProductosfac.getValueAt(i, 3)!=null){
            try {
             ...
             // ideventa idproducto cantidad precio totalProducto descuento 
        factc.ingresardetalleCompra(vent, modeloNat.getValueAt(i, 1).toString() , Double.parseDouble(modeloNat.getValueAt(i, 4).toString()), Double.parseDouble(modeloNat.getValueAt(i, 6).toString()), Double.parseDouble(modeloNat.getValueAt(i, 8).toString()),descu);
        factc.modificarStock(modeloNat.getValueAt(i, 1).toString(), Double.parseDouble(modeloNat.getValueAt(i, 4).toString()));
...
public void calculador(){
   ...
   //  JOptionPane.showMessageDialog(null, "hay  :  ;) "+Nfilas);
    for (int i = 0; i < Nfilas; i++) {
       // JOptionPane.showMessageDialog(null, "hay vamos :  ;) ");
        if (tblProductosfac.getValueAt(i, 3)!=null){
     subtotalc=subtotalc+Double.parseDouble(modeloNat.getValueAt(i, 8).toString());
     listaPorductosc=listaPorductosc+","+modeloNat.getValueAt(i, 1).toString();
     ...
     descuentoc=((Double.parseDouble(modeloNat.getValueAt(i, 7).toString())/100)  *(Double.parseDouble(modeloNat.getValueAt(i, 8).toString())))+descuentoc;  } 
     
        }
       else descuentoc=descuentoc+0.0;
   if (ivabooleanc) ivac=(subtotalc-descuentoc)*0.12;
   else ivac=0;
    totalc=(subtotalc-descuentoc)+ivac;
    }
    txtsubtotal.setText(String.valueOf(fd.format(subtotalc)));
    txtdescuento.setText(String.valueOf(fd.format(descuentoc)));
    txtiva.setText(String.valueOf(fd.format(ivac)));
    txtotal.setText(String.valueOf(fd.format(totalc)));
    // JOptionPane.showMessageDialog(null, "hay vamos :  ;) ");
}
...
public void calcula_cuentas_por_cobrar(){
  //(ID_VENTA, FECHA_REGISTRO, INTERES, VALOR_INTERES, TOTAL_CUENTA, CANT_CUOTAS, VALOR_CUOTAS, FECHA_COBRO, NUM_CUOTA_COBRADA, VALOR_COBRADO, VALOR_POR_COBRAR, ADELANTO)  
   factc.consultarTermino(txttermino.getText());
    ...
    double interes=factc.getinteres(),adelanto=factc.getadelano();
    int Nperiodo=factc.getnperiodo(),Nmese=factc.getNmeses();
    ////////////////////////
    int vent=factc.getidventa();
    double valor_futuro=0,valor_presente,valor_cuotas=0,valor_adelanto=0,valor_interes=0; 
    valor_adelanto=totalc*(adelanto/100);
     
    ///////////////////////////////
    valor_presente=totalc-valor_adelanto;
    ...
    /////////////////////
     ...
     //(ID_VENTA, FECHA_REGISTRO, INTERES, VALOR_INTERES, TOTAL_CUENTA, CANT_CUOTAS, VALOR_CUOTAS, FECHA_COBRO, NUM_CUOTA_COBRADA, VALOR_COBRADO, VALOR_POR_COBRAR, ADELANTO)  
     factc.ingresarCuentaPorPagar(factc.getidventa(),fecha.format(fechaEmision.getDate()) ,interes, valor_interes, valor_futuro, Nperiodo, valor_cuotas, fecha.format(calenEmision.getTime()), 0, 0, valor_futuro, valor_adelanto);
    /////////////////////////////////////////////
    //////////////cuotitas 
    factc.consultarNumeroCuenta_cobrar();
    int numeroCuenta_cobrar=factc.getidcuentaCobrar();
    calenEmision.setTime(fechaEmision.getDate());
    ...
    factc.ingresarCuentaDetalle(numeroCuenta_cobrar, i+1, valor_cuotas, interes,fecha.format(fechaEmision.getDate()),fecha.format(calenEmision.getTime()));     
    }
}
public void nuevo(){
//listaPorductos="";
