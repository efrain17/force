    ...
    {String [] columna ={"Codigo","Descripcion ","Precio","cantidad"};
      ...
      //poner la validacion 
       tabla.setModel(encabezado);
        ...
        String sql =  " SELECT * FROM producto where  '"+Factura.listaPorductos+"' not LIKE CONCAT('%', codigo_producto, '%')";
 ...
 try {sentencia_sql =  miConexion.prepareStatement(sql);
   ...
   //  JOptionPane.showMessageDialog(null, "hay  :  ;) "+Nfilas);
    for (int i = 0; i < Nfilas; i++) {
     ...
     Factura.listaPorductos= Factura.listaPorductos+","+modelo.getValueAt(i, 1).toString();
    ...
    Factura.txtdescuento.setText(String.valueOf(fd.format(descuento)));
...
//*************************************************************************************************
public void ingresarFactura(String fechae,  String fechav,  double sub, double des,double iva,double total, String idcliente,String observacion )
    {   int n1=0;
        String sql2;
        PreparedStatement sentencia_sql2;
        try { sql2 ="insert into venta ( fechaemision, fechavence, subtotal, descuento, iva, totalcobrar, idcliente, observacion)" + "values(?,?,?,?,?,?,?,?)";
...
//*************************************************************************************************
public void ingresardetalleVenta(int idv,  String idpro,  double cant,double pre, double tot,double des)
    {   int n1=0;
        String sql2;
        PreparedStatement sentencia_sql2;
        try { sql2 ="insert into detalleventa ( idventa, idproducto, cantidad, precio, totalProducto, descuento)" + "values(?,?,?,?,?,?)";
          sentencia_sql2 =  miConexion.prepareStatement(sql2);
...
public void autcompleteCliente(String codigo){
 ...
 try {sentencia_sql =  miConexion.prepareStatement(sql);
             ...
             while (rs.next()) { /*
                 ...
                 entrenadorDtos2[2]= rs.getString("apellidos");
                 entrenadorDtos2[3]= rs.getString("direccion");*/
       Factura.textAutoCompleter.addItem(rs.getString("ruc")+"---"+rs.getString("nombres")+""+rs.getString("apellidos"));
...
public void autllenarCliente(String codigo){
 ...
 try {sentencia_sql =  miConexion.prepareStatement(sql);
             ...
             while (rs.next()) { 
                 ...
                 Factura.txtNombreCliente.setText(rs.getString("nombres")+" "+rs.getString("apellidos"));
...
public void autcompleteProducto(String codigo){
 ...
 try {sentencia_sql =  miConexion.prepareStatement(sql);
       ...
       Factura.textAutoCompleterProducto.addItem(rs.getString("codigo_producto")+"---"+rs.getString("descripcion"));
       //textAutoAcompleter.addItem(rs.getString("ruc")+"-"+rs.getString("nombres"));
             }
             rs.close();}
 catch (Exception e) {
...
/////////////////////////////CUENTAS POR COBRAR   //////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////
public void ingresarCuentaPorCobrar(int ID_VENTA, String FECHA_REGISTRO, double INTERES,double VALOR_INTERES,double TOTAL_CUENTA,int CANT_CUOTAS,double VALOR_CUOTAS,String FECHA_COBRO,int  NUM_CUOTA_COBRADA, double VALOR_COBRADO,double  VALOR_POR_COBRAR, double ADELANTO)
    {   int n1=0;
        String sql2;
        PreparedStatement sentencia_sql2;
        try { sql2 ="insert into cuentas_por_cobrar (ID_VENTA, FECHA_REGISTRO, INTERES, VALOR_INTERES, TOTAL_CUENTA, CANT_CUOTAS, VALOR_CUOTAS, FECHA_COBRO, NUM_CUOTA_COBRADA, VALOR_COBRADO, VALOR_POR_COBRAR, ADELANTO)" + "values(?,?,?,?,?,?,?,?,?,?,?,?)";
          sentencia_sql2 =  miConexion.prepareStatement(sql2);
...
public void ingresarCuentaDetalle(int ID_CUENTA_POR_COBRAR,int NUM_CUOTA,double VALOR_CUOTA, double INTERES, String FECHA_EMISION, String FECHA_VENCIMIENTO)
    {   int n1=0;
        String sql2;
        PreparedStatement sentencia_sql2;
        try { sql2 ="insert into cuotas_cobrar_detalle (ID_CUENTA_POR_COBRAR, NUM_CUOTA, VALOR_CUOTA, INTERES, FECHA_EMISION, FECHA_VENCIMIENTO)" + "values(?,?,?,?,?,?)";
          sentencia_sql2 =  miConexion.prepareStatement(sql2);
...
public void consultarNumeroCuenta_cobrar()
    {  
        String sql = "SELECT MAX(ID_CUENTA_POR_COBRAR) FROM cuentas_por_cobrar ";
        PreparedStatement sentencia_sql = null;
        ResultSet rs = null;
 try {sentencia_sql =  miConexion.prepareStatement(sql);
             rs =  sentencia_sql.executeQuery(sql);
